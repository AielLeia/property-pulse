...
const PropertyEditForm = () => {
  ...
  const setEmptyStringForNullField = (propertyData) => {
    Object.keys(propertyData).forEach((key) => {
      if (propertyData[key] === null) {
        propertyData[key] = '';
      }
      if (typeof propertyData[key] === 'object') {
        setEmptyStringForNullField(propertyData[key]);
      }
    });
  };
  ...
  useEffect(() => {
    const fetchPropertyData = async () => {
      try {
        ...
        setEmptyStringForNullField(propertyData.properties);
        setFields(propertyData.properties);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedResponse = await fetch(`/api/properties/${id}`, {
        method: 'PUT',
        body: formData,
      });
      ...
      } else if ([401, 403].includes(updatedResponse.status)) {
